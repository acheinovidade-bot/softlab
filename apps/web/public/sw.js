const CACHE = 'erp-hibrido-shell-v1';
...
self.addEventListener('sync', (event) => { if (event.tag === 'pos-checkout-sync') event.waitUntil(self.clients.matchAll({ type: 'window' }).then((clients) => clients.forEach((client) => client.postMessage({ type: 'pos-sync-request' })))); });
