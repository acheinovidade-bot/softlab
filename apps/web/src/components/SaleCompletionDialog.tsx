...
async function print80mm(receipt: SaleReceipt, fiscal?: FiscalDocument) {
  ...
  const creditDetails = receipt.credit
    ? `<section class="credit-account"><b>VENDA NO CREDIÁRIO</b><div class="summary-row"><span>Cliente</span><b>${escape(receipt.credit.customerName)}</b></div><div class="summary-row"><span>Valor desta venda</span><b>${decimalMoney(receipt.credit.saleCreditAmount)}</b></div><div class="summary-row credit-total"><span>CONSTA EM ABERTO</span><b>${receipt.credit.totalOpenAmount === null ? 'ATUALIZA APÓS SINCRONIZAR' : decimalMoney(receipt.credit.totalOpenAmount)}</b></div><small>Valor total devido pelo cliente, incluindo esta compra.</small></section><div class="dash"></div>`
    : '';
