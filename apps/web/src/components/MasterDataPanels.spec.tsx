...
vi.mock('../api', () => ({ apiRequest: vi.fn((path: string) => Promise.resolve(path.includes('/enrichment/cnpj/') ? { cnpj: '11222333000181', found: true, provider: 'brasilapi', fields: { legalName: 'Empresa sugerida', tradeName: 'Sugestão', phone: null, email: null, registrationStatus: 'ATIVA', address: { postalCode: '60123000', street: 'Rua Central', number: '10', district: 'Centro', city: 'Fortaleza', state: 'CE', country: 'BR' } }, sourceUrl: 'https://brasilapi.com.br/api/cnpj/v1/11222333000181', warnings: ['Confirme os dados.'] } : path === '/master/customers/customer' ? { id: 'customer', personType: 'F', taxId: '52998224725', legalName: 'Ana Cliente', tradeName: null, phone: null, whatsapp: '85999999999', email: null, creditLimit: '500.0000', active: true, addresses: [{ type: 'main', isDefault: true, postalCode: '60123000', street: 'Rua Cliente', number: '10', complement: null, district: 'Centro', city: 'Fortaleza', state: 'CE', country: 'BR' }] } : path === '/master/supplier-products/catalog' ? [{ id: 'product', code: 'P1', description: 'Produto vinculado' }] : path.includes('/master/supplier-products/supplier/') ? [] : path.startsWith('/master/suppliers') ? { items: [{ id: 'supplier', taxId: '11222333000181', legalName: 'Fornecedor Exemplo', tradeName: null, email: null, phone: null, averageLeadDays: 5, paymentTerms: null, active: true }], total: 1, page: 1, pageSize: 20 } : { items: [{ id: 'customer', personType: 'F', taxId: '52998224725', legalName: 'Ana Cliente', tradeName: null, phone: null, whatsapp: '85999999999', email: null, creditLimit: '500.0000', active: true }], total: 1, page: 1, pageSize: 20 })) }));
...
describe('CustomersPanel', () => {
  ...
  it('allows changing an existing customer from natural to legal person', async () => {
    ...
    const type = await screen.findByRole('combobox', { name: 'Tipo de pessoa' });
