...
export class StockService {
  ...
  async overview(auth: AccessTokenPayload, query: unknown) {
    const page = stockListSchema.parse(query);
    ...
    const items = products.map((product) => {
      const productBalances = balances.filter(({ productId }) => productId === product.id);
      const quantity = productBalances.reduce(
        (sum, balance) => sum.add(balance.quantity),
        new Prisma.Decimal(0),
      );
      const reserved = productBalances.reduce(
        (sum, balance) => sum.add(balance.reservedQuantity),
        new Prisma.Decimal(0),
      );
      const available = quantity.sub(reserved);
      const setting = settings.find(({ productId }) => productId === product.id);
      const minimum = setting?.minimumStock ?? new Prisma.Decimal(0);
      return {
        ...product,
        quantity,
        ...
        status: quantity.lte(0)
          ? ('out' as const)
          : available.lte(minimum)
            ? ('low' as const)
            : ('ok' as const),
      };
    });
    return {
      ...
      summary: {
        out: items.filter(({ status }) => status === 'out').length,
        low: items.filter(({ status }) => status === 'low').length,
        ok: items.filter(({ status }) => status === 'ok').length,
      },
    };
  }
  ...
  async lookups(auth: AccessTokenPayload) {
    ...
    const [locations, products, lots] = await Promise.all([
      this.prisma.stockLocation.findMany({
        where: { companyId: auth.companyId, warehouseId: { in: warehouseIds } },
        orderBy: { name: 'asc' },
      }),
      this.prisma.product.findMany({
        where: { companyId: auth.companyId, deletedAt: null, active: true },
        orderBy: { description: 'asc' },
        take: 100,
        select: {
          id: true,
  ...
  async movements(auth: AccessTokenPayload, query: unknown) {
    const page = movementListSchema.parse(query);
    const where: Prisma.StockMovementWhereInput = {
      companyId: auth.companyId,
      branchId: auth.branchId,
      ...(page.productId ? { productId: page.productId } : {}),
      ...(page.movementType ? { movementType: page.movementType } : {}),
    };
    ...
    const locationIds = [...new Set(items.map(({ locationId }) => locationId))];
    const lotIds = items.flatMap(({ lotId }) => (lotId ? [lotId] : []));
    const actorIds = [...new Set(items.flatMap(({ createdBy }) => (createdBy ? [createdBy] : [])))];
    const [products, locations, lots, actors] = await Promise.all([
      this.prisma.product.findMany({
        where: { companyId: auth.companyId, id: { in: productIds } },
        select: { id: true, code: true, description: true },
      }),
      this.prisma.stockLocation.findMany({
        where: { companyId: auth.companyId, id: { in: locationIds } },
    ...
    return {
      items: items.map((item) => ({
        ...
        location: locations.find(({ id }) => id === item.locationId),
        lot: lots.find(({ id }) => id === item.lotId) ?? null,
        actor: actors.find(({ id }) => id === item.createdBy),
      })),
      total,
      page: page.page,
      pageSize: page.pageSize,
  ...
  async lots(auth: AccessTokenPayload, query: unknown) {
    const page = lotListSchema.parse(query);
    const today = this.today();
    const deadline = new Date(today);
    if (page.status !== 'all' && page.status !== 'expired')
      deadline.setUTCDate(deadline.getUTCDate() + Number(page.status));
    const statusFilter =
      page.status === 'expired'
        ? Prisma.sql`l.expires_at < ${today}`
        : page.status === 'all'
          ? Prisma.sql`TRUE`
          : Prisma.sql`l.expires_at >= ${today} AND l.expires_at <= ${deadline}`;
    type Row = {
      id: string;
      productId: string;
      lotNumber: string;
    ...
    return {
      items: rows.map((row) => ({
        ...
        status: this.expiryStatus(row.expiresAt, today),
      })),
      total: rows[0]?.total ?? 0,
      page: page.page,
      pageSize: page.pageSize,
      summary: summaryRows[0] ?? { expired: 0, within15: 0, within30: 0, within60: 0, within90: 0 },
    };
  }

  async fefo(auth: AccessTokenPayload, productId: string, query: unknown) {
    const input = fefoSchema.parse(query);
    const product = await this.product(auth.companyId, productId);
    if (!product.controlsLot) throw new ConflictException('Produto não controla lote');
    if (input.locationId) await this.location(auth, input.locationId);
    ...
    const lots = await this.prisma.stockLot.findMany({
      where: { companyId: auth.companyId, productId, id: { in: lotIds } },
    });
    const today = this.today();
    const ordered = lots
      .filter(({ expiresAt }) => (expiresAt ? expiresAt >= today : !product.controlsExpiry))
      .sort((left, right) =>
        left.expiresAt === null
          ? 1
    ...
    return {
      ...
      requestedQuantity: new Prisma.Decimal(input.quantity),
      allocations,
      shortageQuantity: remaining,
      fulfilled: remaining.eq(0),
      warning: 'Prévia FEFO: nenhuma reserva ou movimentação foi criada.',
    };
  }

  async createLot(auth: AccessTokenPayload, input: unknown) {
    const data = createLotSchema.parse(input);
    const product = await this.product(auth.companyId, data.productId);
    if (!product.controlsLot)
      throw new ConflictException('O produto não está configurado para controlar lote');
    if (product.controlsExpiry && !data.expiresAt)
      throw new ConflictException('Validade obrigatória para este produto');
    const now = new Date();
    try {
      return await this.prisma.stockLot.create({
        data: {
          id: uuidV7(),
          companyId: auth.companyId,
          ...data,
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002')
        throw new ConflictException('Lote já cadastrado para este produto');
      throw error;
    }
  }
  ...
  async adjust(auth: AccessTokenPayload, input: unknown) {
    ...
    const product = await this.product(auth.companyId, data.productId);
    const location = await this.location(auth, data.locationId);
    if (product.controlsLot && !data.lotId)
      throw new ConflictException('Lote obrigatório para este produto');
    if (!product.controlsLot && data.lotId)
      throw new ConflictException('Produto sem controle de lote não aceita lote');
    if (
      data.lotId &&
      !(await this.prisma.stockLot.findFirst({
        where: { id: data.lotId, companyId: auth.companyId, productId: product.id },
      }))
    )
      throw new NotFoundException('Lote não encontrado para o produto');
    const direction = ['entry', 'adjustment_in', 'return_in'].includes(data.movementType) ? 1 : -1;
    const delta = new Prisma.Decimal(data.quantity).mul(direction);
    let attempt = 0;
    while (attempt < 3) {
      try {
        return await this.prisma.$transaction(
          async (tx) => {
            ...
            const quantity = (current?.quantity ?? new Prisma.Decimal(0)).add(delta);
            const reserved = current?.reservedQuantity ?? new Prisma.Decimal(0);
            if (!product.allowsNegativeStock && quantity.lt(reserved))
              throw new ConflictException('Movimentação deixaria o estoque disponível negativo');
            const balance = current
              ? await tx.stockBalance.update({
                  where: { id: current.id },
                  data: { quantity, version: { increment: 1 }, updatedAt: now },
                })
            ...
            await tx.auditLog.create({
              data: {
                ...
                beforeData: this.json(current),
                afterData: this.json({ movement, balance, reason: data.reason }),
                occurredAt: now,
                createdAt: now,
                updatedAt: now,
              },
  ...
  private async location(auth: AccessTokenPayload, id: string) {
    ...
    if (!warehouse) throw new NotFoundException('Localização não pertence à filial atual');
    return location;
  }
  private today(): Date {
  ...
  private expiryStatus(
    expiresAt: Date | null,
    today: Date,
  ): 'expired' | '15' | '30' | '60' | '90' | 'valid' | 'none' {
